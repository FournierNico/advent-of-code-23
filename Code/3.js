...
let arrayInput = [];
input.forEach((row) => {
  arrayInput.push(Array.from(row));
});
...
for (i = 0; i < arrayInput.length; i++) {
  ...
  for (j = 0; j < arrayInput[i].length; j++) {
    if (alreadyChecked[i][j] === undefined && arrayInput[i][j].match(/\d/g)) {
      alreadyChecked[i][j] = true;
      let indice = [];
      indice[0] = j;
      for (h = j + 1; h < arrayInput[i].length; h++) {
        if (!arrayInput[i][h].match(/\d/g)) {
          break;
        }
        alreadyChecked[i][h] = true;
        indice.push(h);
      }
      console.log(indice);
    }
