...
input.forEach((val) => {
  ...
  let finalNumber = "";
  if (numbers) {
    finalNumber = numbers[0] + numbers[numbers.length - 1];
...
function remplacerChiffresEnLettres(str) {
  ...
  return str.replace(
    ...
    (match) => {
      return chiffresEnLettres[match];
    }
  );
}

const findFirst = (stringValue) => {
  let chaine = "";
  for (i = 0; i < stringValue.length; i++) {
    chaine += stringValue[i];
    let numberFound = chaine.match(
      /(\d|one|two|three|four|five|six|seven|eight|nine)/g
    );
    if (numberFound) {
      numberFound = remplacerChiffresEnLettres(numberFound[0]);
      return numberFound;
    }
  }
};

const findLast = (stringValue) => {
  let chaine = "";
  for (i = stringValue.length - 1; i >= 0; i--) {
    chaine = stringValue[i] + chaine;
    console.log(chaine);
    let numberFound = chaine.match(
      /(\d|one|two|three|four|five|six|seven|eight|nine)/g
    );
...
input.forEach((inputVal) => {
  const numberValue = Number(findFirst(inputVal) + findLast(inputVal));
  total2 += numberValue;
});
