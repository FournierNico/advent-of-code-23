...
//clean input
input.forEach((game) => {
  let entry = game.split(": ")[1];
  entry = entry.split("; ");

  let cleanedGame = [[]];

  for (i = 0; i < entry.length; i++) {
    let entry2 = entry[i].split(", ");
    cleanedGame[i] = [];
    for (j = 0; j < entry2.length; j++) {
      let entry3 = entry2[j].split(" ");
      cleanedGame[i][j] = entry3;
    }
  }

  cleanedInput.push(cleanedGame);
});
...
//puzle 1
let addGamesValide = 0;
for (i = 0; i < cleanedInput.length; i++) {
  let gameValide = true;
  for (const sousGame of cleanedInput[i]) {
    let compte = { red: 0, green: 0, blue: 0 };

    for (const [nombre, couleur] of sousGame) {
      compte[couleur] += parseInt(nombre, 10);
    }

    const estValide = Object.keys(compte).every(
      (couleur) => compte[couleur] <= limites[couleur]
    );
...
let power = 0;
for (i = 0; i < cleanedInput.length; i++) {
  let maxCompte = { red: 0, green: 0, blue: 0 };
  for (const sousGame of cleanedInput[i]) {
    for (const [nombre, couleur] of sousGame) {
      if (maxCompte[couleur] <= parseInt(nombre, 10)) {
        maxCompte[couleur] = parseInt(nombre, 10);
      }
    }
  }
  let sousValue = 1;
  for (const [couleur, value] of Object.entries(maxCompte)) {
    sousValue = sousValue * value;
  }
  power += sousValue;
}

console.log(addGamesValide);
console.log(power);
